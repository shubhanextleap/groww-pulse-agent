...
- Ingest public reviews from the last 8–12 weeks (configurable window) from Google Play via a dedicated Play Store Reviews MCP server (which we will build in this project).
...
The agent is an MCP host/client; it will use a Play Store Reviews MCP server (provided in this project) to retrieve reviews, and it does not embed Google credentials or call the Docs/Gmail REST APIs directly for delivery.
